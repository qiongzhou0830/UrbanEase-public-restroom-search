...
export default function App() {
  ...
  const remove = async (id) => {
    if (!currentUser) {
      alert("Login as manager to delete.");
      return;
    }
    if (currentUser.role !== "admin") {
      alert("Only managers can delete restrooms.");
      return;
    }
    if (!confirm(`Delete restroom ${id}?`)) return;
  
    await fetch(`${API}/api/restrooms/${id}`, {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
  ...
  async function saveEdit(rid) {
    ...
    if (!res.ok) {
        if (data.error === "CONCURRENT_MODIFICATION") {
            alert(`This data has been modified by another user.  The page will refresh automatically.`);
            await load();
            setEditingId(null);
            return;
        } else if (data.error === "VERSION_REQUIRED") {
      ...
      <div style={{ display: "grid", gridTemplateColumns: "360px 1fr" }}>
        ...
        >
          ...
          >
            ...
            {!currentUser && (
              <form
                onSubmit={authMode === "login" ? doLogin : doSignup}
        ...
        <div style={{ position: "relative", zIndex: 1 }}>
          ...
          >
            ...
            {enriched.map((r) => {
              ...
              return (
                <Marker key={r.restroom_id} position={[lat, lng]} icon={pin}>
                  <Popup>
                    {editingId === r.restroom_id ? (
                      ...
                      <div style={{ minWidth: 220, display: "grid", gap: 8 }}>
                        ...
                        <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
                          <button onClick={() => saveEdit(r.restroom_id)}>
                            Save
                          </button>
                      ...
                      <div style={{ marginTop: 8, paddingTop: 4, borderTop: "1px solid #eee" }}>
                        <div style={{ fontSize: 12, marginBottom: 4 }}>Rate this restroom</div>
                        <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
                          <select
                            value={reviewDraft.rating}
                            onChange={(e) =>
                              setReviewDraft((d) => ({ ...d, rating: e.target.value }))
                            }
                          >
                          ...
                          <button onClick={() => submitReview(r.restroom_id)}>Submit</button>
                        </div>
                        <input
                          style={{
                            marginTop: 4,
